#ifndef ATENTS_CHARACTER_LIT_INPUT_INCLUDED
#define ATENTS_CHARACTER_LIT_INPUT_INCLUDED
...
float _ShadowSmooth;
float _ShadowSpherizing;

half4 _FresnelColor;
float _FresnelThreshold;
float _FresnelSmooth;
float _FresnelSpherizing;

half4 _SpecularColor;
float _SpecularThreshold;
float _SpecularSmooth;
float _SpecularSpherizing;

float _EnvironmentLights;

float _GIIntensity;
float _GIThreshold;
float _GISmooth;

float3 _VertexAveragePos;
CBUFFER_END
...
TEXTURE2D(_ShadowCasterMask);   SAMPLER(sampler_ShadowCasterMask);
